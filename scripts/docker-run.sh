...
CTR=""
for _cmd in docker podman nerdctl; do
...
if [ -z "$CTR" ]; then
  echo ""
  printf "${BRED}  ╭─────────────────────────────────────────────╮${R}\n"
  printf "${BRED}  │${R}  컨테이너 런타임을 찾을 수 없습니다.       ${BRED}│${R}\n"
  printf "${BRED}  │${R}                                             ${BRED}│${R}\n"
  printf "${BRED}  │${R}  다음 중 하나를 설치하세요:                 ${BRED}│${R}\n"
  printf "${BRED}  │${R}  ${SILVER}• Docker${R}    ${DIM}https://docs.docker.com${R}      ${BRED}│${R}\n"
  printf "${BRED}  │${R}  ${SILVER}• Podman${R}    ${DIM}https://podman.io${R}             ${BRED}│${R}\n"
  printf "${BRED}  │${R}  ${SILVER}• nerdctl${R}   ${DIM}https://github.com/containerd${R} ${BRED}│${R}\n"
  printf "${BRED}  │${R}                                             ${BRED}│${R}\n"
  printf "${BRED}  │${R}  또는 Docker 없이 직접 실행:                ${BRED}│${R}\n"
  printf "${BRED}  │${R}  ${DIM}./scripts/baremetal-run.sh${R}                ${BRED}│${R}\n"
  printf "${BRED}  ╰─────────────────────────────────────────────╯${R}\n"
  echo ""
  exit 1
fi
...
if ! $CTR info > /dev/null 2>&1; then
  echo ""
  err "${CTR}에 접근할 수 없습니다."
  echo ""
  if [ "$CTR" = "docker" ]; then
    printf "${BRED}  ╭─────────────────────────────────────────────╮${R}\n"
    printf "${BRED}  │${R}  Docker 데몬이 실행 중인지 확인하세요.      ${BRED}│${R}\n"
    printf "${BRED}  │${R}                                             ${BRED}│${R}\n"
    printf "${BRED}  │${R}  ${DIM}sudo systemctl start docker${R}              ${BRED}│${R}\n"
    printf "${BRED}  │${R}                                             ${BRED}│${R}\n"
    printf "${BRED}  │${R}  권한 문제라면:                              ${BRED}│${R}\n"
    printf "${BRED}  │${R}  ${DIM}sudo usermod -aG docker \$USER${R}            ${BRED}│${R}\n"
    printf "${BRED}  │${R}  ${DIM}(재로그인 필요)${R}                           ${BRED}│${R}\n"
    printf "${BRED}  ╰─────────────────────────────────────────────╯${R}\n"
  else
    printf "${BRED}  ╭─────────────────────────────────────────────╮${R}\n"
    printf "${BRED}  │${R}  ${CTR} 서비스가 실행 중인지 확인하세요.   ${BRED}│${R}\n"
    printf "${BRED}  ╰─────────────────────────────────────────────╯${R}\n"
  fi
  echo ""
  exit 1
...
if [ "${1}" = "--logs" ]; then
  ...
  HINT_PID=$!
  CTR="$CTR" NAMBI_CONTAINER="$CONTAINER_NAME" python3 -c "
...
p = subprocess.Popen([os.environ.get('CTR','docker'),'logs','-f',c], preexec_fn=os.setsid)
def h(s,f):
    try: p.kill()
    except: pass
    os._exit(0)
...
" || $CTR logs -f "$CONTAINER_NAME" 2>&1 || true
...
log "컨테이너 런타임: ${SILVER}${CTR}${R}"
log "프로젝트 경로: ${SILVER}${ROOT_DIR}${R}"
log "데이터 디렉토리: ${SILVER}${NAMBI_DIR}${R}"

mkdir -p "$NAMBI_DIR"

log "컨테이너 이미지 빌드 중: ${CYAN}${IMAGE_NAME}${R}"
printf "${DIM}  ╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌${R}\n"
$CTR build -f "$ROOT_DIR/docker/Dockerfile" \
  --build-arg "CACHEBUST_YTDLP=$(date +%s)" \
...
if $CTR ps -a --format '{{.Names}}' | grep -qx "$CONTAINER_NAME" 2>/dev/null; then
  _status=$($CTR inspect --format '{{.State.Status}}' "$CONTAINER_NAME" 2>/dev/null || echo "unknown")
  log "기존 컨테이너 종료 및 제거: ${CYAN}${CONTAINER_NAME}${R}  (${_status})"
  $CTR rm -f "$CONTAINER_NAME" > /dev/null
  ok "기존 컨테이너 제거 완료"
fi
...
kill $HINT_PID 2>/dev/null || true; wait $HINT_PID 2>/dev/null || true
printf "${SILVER}--- ─────────────────────────────────────── ---${R}\n"
if $CTR ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$" 2>/dev/null; then
  ok "로그 보기 종료. 봇은 계속 실행 중입니다."
else
  EXIT_CODE=$($CTR inspect --format '{{.State.ExitCode}}' "$CONTAINER_NAME" 2>/dev/null || echo "?")
  err "봇이 종료되었습니다. (exit ${EXIT_CODE})"
  $CTR rm "$CONTAINER_NAME" > /dev/null 2>&1 || true
  echo ""
  if [ "$EXIT_CODE" = "0" ]; then
    printf "${BYELLOW}  ╭─────────────────────────────────────────────╮${R}\n"
